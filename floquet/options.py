...
class Options(Serializable):
    ...
    Parameters:
        fit_range_fraction: Fraction of the amplitude range to sweep over before changing
            the definition of the bare state to that of the fitted state from the previous
            range. For instance if fit_range_fraction=0.4, then the amplitude range is
            split up into three chunks: the first 40% of the amplitude linspace, then from
            40% -> 80%, then from 80% to the full range. For the first fraction of
            amplitudes, they are compared to the bare eigenstates for identification. For
            the second range, they are compared to the fitted state from the first range.
            And so on. Defaults to 1.0, indicating that no iteration is performed.
        floquet_sampling_time_fraction: What point of the drive period we want to sample the
            Floquet modes. Defaults to 0.0, indicating the floquet modes at t=0*T where T is
            the drive period.
        ...
        nsteps: QuTiP integration parameter, number of steps the solver can take. Defaults
            to 30_000.
        num_cpus: Number of cpus to use in parallel computation of Floquet modes over the
